...
V2 不否定现有 `speaker_labeling_plan.md` 和阶段 2 structured pipeline。它是一条新的对照路线：用更接近人类阅读过程的顺序状态机做说话人标注，然后和当前 structured 方案比较准确率、复核量和明显误标。
...
- 很多台词在刚出现时不能立刻精确命名，但可以先判断“不是某个重要角色”“像村民群体”“像同一轮对话中的另一方”。
...
- `npc_group`: 村民、群众、路人、商队成员、卫兵群体等不重要群体。
...
- 被称呼者通常不是说话者。
- “罗伦斯先生你”这类表达应作为“不是罗伦斯本人”的强反证。
- 如果只能判断“村民/群众/路人”，应输出 `npc_group`，不要硬建 mystery。
...
```json
{
  ...
  "active_entities": ["char_lawrence", "npc_group_villagers"],
...
```json
{
  "updates": [
    {
      "entity_id": "char_lawrence",
      "summary": "二十五岁的旅行商人，正在离开深山村落",
      "importance": "major",
      "dialogue_count_delta": 2,
  ...
  "repairs": [
    {
      ...
      "new_speaker_entity_id": "npc_group_villagers",
      "speaker_status": "npc_group",
      "confidence": 0.82,
      "reason": "交易场景中与罗伦斯轮流说话的村民方",
...
这样 V2 可以和现有方案并行对比，不会互相覆盖 cache。
...
- 能确定是罗伦斯，就标罗伦斯。
- 能确定不是罗伦斯但只是村民，就标 `npc_group_villagers`。
...
如果 V2 在这些方面优于 structured 方案，就继续实现更完整的回填修正。若 V2 更慢但更准，可以接受；若更慢且误标更多，则回到旧 structured pipeline 继续优化。
